...
rotate_coords_by_radians <- function(coords, radians, axis = NULL) {
  ...
  if(ncol(coords) == 2){
    ...
    if(is.null(axis)) stop("Rotation axis must be specified for 3D rotations as either 'x', 'y' or 'z'.", call. = FALSE)
...
reflect_coords_in_axis <- function(coords, axis){
  ...
  if(!dimensions %in% 2:3)           stop("Reflection is only supported for coordinates of 2 or 3 dimensions.", call. = FALSE)
  if(missing(axis))                  stop("Reflection axis must be provided.", call. = FALSE)
  if(dimensions == 2 && axis == "z") stop("Axis must be one of 'x' or 'y'.", call. = FALSE)
  if(!axis %in% c("x", "y", "z"))    stop("Invalid reflection axis provided.", call. = FALSE)
