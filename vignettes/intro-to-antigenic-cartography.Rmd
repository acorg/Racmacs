...
Depending on how much data goes into making a map it can be more or less vulnerable to outliers in the data. Even maps with quite a lot of data can be vulnerable to for example the odd sera that justs reacts to everything in the assay for example. This is a good thing to look for and be aware of.
...
[multi-dimensional scaling](https://en.wikipedia.org/wiki/Multidimensional_scaling#Implementations) (MDS).
...
2. "relax" the map from these starting conditions using the [conjugate gradient method](https://en.wikipedia.org/wiki/Conjugate_gradient_method).
